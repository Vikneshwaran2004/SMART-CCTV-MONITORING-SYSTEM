...
# Create a directory to store motion-detected images if it doesn't exist
...
def rect_noise():
    ...
    while True:
        ...
        # Define the region for motion detection based on user selection
        ...
        if len(contours) > 0:
            ...
            # Play sound if motion is detected
            if not pygame.mixer.music.get_busy():  # To avoid overlapping sounds
                pygame.mixer.music.play()

            # Save the frame with motion detection to file
            timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
            image_path = f"motion detection/motion_{timestamp}.jpg"
            cv2.imwrite(image_path, frame1)
            print(f"Motion detected and image saved at {image_path}")
